...
#binding 
#data2 <- cbind(data, newdata, by = c("ID", "study")) #DOESN'T WORK
...
Folds = createFolds(Bear$ID, k=5, list=TRUE, returnTrain = FALSE)  
...
for( B in Folds){
  ...
  #recreate our models
  modelX <- glmer(diagnosis ~ rangeRE + (1|ID) + (1|study), data = Train, family= "binomial", REML=F)
  ...
  #sensitivity & specificity  
  Train$diagnosis <- as.factor(data$diagnosis)
  Train$prediction <- as.factor(data$prediction)
  Test$diagnosis <- as.factor(data$diagnosis)
  Test$prediction <- as.factor(data$prediction)

  sensitivityTrain[N] <- sensitivity(data = Train$prediction, reference = Train$diagnosis, positive =
  "1") 

  specificityTrain[N] <- specificity(data = Train$prediction, reference = Train$diagnosis, negative =
  "0") 
  ...
  specificityTest[N] <-specificity(data = data$prediction, reference = data$diagnosis, negative =
  "0") 

  posPredValueTest[N] <- posPredValue(data = data$prediction, reference = data$diagnosis,
  positive = "1") 

  negPredValueTest[N] <-negPredValue(data = data$prediction, reference = data$diagnosis,
  negative = "0")
