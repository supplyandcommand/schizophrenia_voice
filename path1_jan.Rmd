...
```{r}
a <- buildmer(diagnosis ~ mean + sd + range + iqr + 
...
best_model <- glmer(diagnosis ~ se + mean + iqr + mean_abs + 
                      coef_var + Gender + (1 | study), Bear_scaled, family = "binomial")
...
Bear_best %>%
  ...
  ggplot(aes(diagnosis, pred_best_num, color = factor(pred_best))) +
  geom_jitter() +
  theme_bw() +
 ...
 Bear_best %>%
  ...
  theme_bw() +
  labs(title = "Model performance", subtitle = "diagnosis ~ se + mean + iqr + mean_abs + coef_var + Gender + (1 | study)", fill = "model prediction", x = "true diagnosis")
```


## brute force open source
NOT WORKING
```{r phack brute}
