...
mean_par_melt <- mean_par %>%
  ...
  mutate(model_type = 
           case_when(
             model == "diagnosis ~ iqr" ~ "m1",
             model == "diagnosis ~ mean + range + iqr + (1|study)" ~ "m2",
             model == "diagnosis ~ mean + sd + range + mean_abs + se + Gender + (1|study)" ~ "m3",
             model == "diagnosis ~ mean_abs + coef_var + se + Gender" ~ "m4")) %>%
  mutate(model_type_m = 
           case_when(
             model == "diagnosis ~ iqr" ~ paste0(model_type, ": ", model),
             model == "diagnosis ~ mean + range + iqr + (1|study)" ~ paste0(model_type, ": ", model),
             model == "diagnosis ~ mean + sd + range + mean_abs + se + Gender + (1|study)" 
             ~ paste0(model_type, ": ", model),
             model == "diagnosis ~ mean_abs + coef_var + se + Gender" ~ paste0(model_type, ": ", model)))
